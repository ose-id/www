...
interface PrismProps {
  ...
  timeScale?: number;
  monochrome?: boolean;
  hueTransition?: boolean;
  hueTransitionSpeed?: number;
}
...
function setup() {
  ...
  const BASE_SAT = props.transparent ? 1.5 : 1;
  const SAT = props.monochrome ? 0 : BASE_SAT;
  const HUE_TRANS = props.hueTransition && props.monochrome ? 1 : 0;
  const HUE_TRANS_SPEED = Math.max(0.01, props.hueTransitionSpeed || 0.1);
  ...
  const fragment = /* glsl */ `
      ...
      uniform int   uHueTransition;
      uniform float uHueTransSpeed;
      ...
      void main(){
        ...
        float satVal = uSaturation;
        if (uHueTransition == 1) {
          // Animate saturation: 0 -> max -> 0 in a smooth loop
          satVal = uBaseSaturation * (0.5 + 0.5 * sin(iTime * uHueTransSpeed));
        }
        col = clamp(mix(vec3(L), col, satVal), 0.0, 1.0);
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uTimeScale: { value: TS },
      uHueTransition: { value: HUE_TRANS },
      uHueTransSpeed: { value: HUE_TRANS_SPEED },
      uBaseSaturation: { value: BASE_SAT },
    },
  });
